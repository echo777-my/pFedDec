...
from system.flcore.clientdec import *
...
class pFedDec:
    def __init__(self, args, times):
        ...
        personalization_dim =512
        cs = GatingMechanism(in_dim, in_dim,personalization_dim).to(args.device)

        for i in range(self.num_clients):
            train_data = read_client_data(self.dataset, i, is_train=True)
            test_data = read_client_data(self.dataset, i, is_train=False)
            client = clientdec(args, 
                            id=i, 
    ...
    def train(self):
        ...
        for client in self.clients:
            personalized_model_path =os.path.join('1', f'client_{client.id}-personalized.pth')
            generic_model_path =os.path.join('1', f'client_{client.id}-generic.pth')
            if os.path.exists(personalized_model_path):
                os.remove(personalized_model_path)
                print(f"Deleted personalized vector generator parameters for client {client.id}")
            if os.path.exists(generic_model_path):
                os.remove(generic_model_path)
                print(f"Deleted generic vector generator parameters for client {client.id}")
...
class GatingMechanism(nn.Module):          
    def __init__(self, input_size, hidden_size, personalization_dim):
        super(GatingMechanism, self).__init__()
        self.fc_global = nn.Linear(input_size + personalization_dim, hidden_size)
        self.fc_personal = nn.Linear(input_size + personalization_dim, hidden_size) 
        self.fc_personal_out = nn.Linear(hidden_size, input_size)
    
    def forward(self, rep, personalization_vector, generic_vector):
        x_combined1 = torch.cat((rep, personalization_vector), dim=-1)       
        x_combined2 = torch.cat((rep, generic_vector), dim=-1)
       
        personal_hidden = torch.sigmoid(self.fc_personal(x_combined1))
        global_hidden = torch.sigmoid(self.fc_global(x_combined2))
        
        gate_personal = torch.sigmoid(self.fc_personal_out(personal_hidden))
        gate_global = torch.sigmoid(self.fc_global_out(global_hidden))
        return gate_global, gate_personal
